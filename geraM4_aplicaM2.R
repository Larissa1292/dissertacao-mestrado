...
m4_loglik <- function(theta, w, y){
  ...
  #### Definindo o resultado da Esperança Condicional, que no caso do M2 é o probit ####
  
  probit <- pnorm(theta[3] + theta[4] * w) #Aplica dados do M2 no M4
...
for(i in 1:R){
  ...
  tryCatch(  {
    otimizacao <- optimParallel(
      ...
      par = c(0.009, 0.009, 0.001, 0.99, 0.0009), #chutes iniciais para esquema 1
      fn = m4_loglik,
      method = "L-BFGS-B",
