...
parametros <- c(pi0, pi1, beta0, beta1) #Aqui nao tem lambda, pq o modelo 2 so tem sem erro de class.
...
m2_loglik <- function(theta, w, y){
  ...
  prob <- pnorm(theta[1] + theta[2] * w) # Aplicando M1 nos dados do M2
...
write.csv(x = resultado, file = paste0("geraM2_aplicaM1_r", R,"_n", n, "_l", lambda,"_sig", sig,".csv"))
